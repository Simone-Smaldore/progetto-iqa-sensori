...
tresh=38;%64;
filter_siz=9;%10 migliore % 20 di default

%Interpolation?
[Z , M] = interp1k ( Z , M , (Z(1):0.1:Z(length(Z))+0.01)' , 3 ); 
...
%surf_m = medianoAdattivo(surf,8);
filled=imfill(surf,'holes');
surf(surf==0)=filled(surf==0);
...
figure
view(310+180,40); % visualizzazione distanza immagine
...
set(gca,'Color','w',...
    ...
    'YDir','reverse',...
    'ZDir','reverse',...
    'CameraPosition',1.5*[-280 -96 -615]);
   % 'CameraPosition',[0 -1 -615]);%frontale
   %'CameraPosition',[0 60 -615]); %MAx Vista
axis tight;
xlabel('y[mm]');
ylabel('x[mm]');
zlabel('z[mm]');
% save data1 M X Y Z % per memorizzare la nuova matrice 3D (messo io)
